...
import { STORAGE_PRICING } from "@/lib/constant";
...
export async function createPaddleCheckout(userId: string, priceId: string) {
  try {
    const response = await axios.post(
      "https://api.paddle.com/checkout/sessions",
      {
        customer_id: userId, // or your internal user id mapped to Paddle
        price_id: priceId,   // this is your Paddle price id
      },
      {
        headers: {
          Authorization: `Bearer ${PADDLE_API_KEY}`,
