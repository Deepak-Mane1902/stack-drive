...
export interface ISubscription {
  ...
  billing: {
    cycle: "month" | "year";
    billingStart?: Date; // Optional: Billing start date
    billingEnd?: Date;   // Optional: Billing end date
  };
...
const subscriptionSchema: Schema<SubscriptionModel> = new Schema(
  {
    ...
    selectedStorage: { 
      type: Number, 
      required: true, 
      default: 16105174016 // Default: 16 GB in Bytes
    },
    usedStorage: { 
      type: Number, 
      required: true, 
      default: 0 // Initially no storage used
    },
    pricePerGB: { 
      type: Number, 
      default: 0.23 // $0.23 per GB
    },
    totalPrice: {
      type: Number,
      required: true,
